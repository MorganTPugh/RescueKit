...
// Per-template/aspect font-size caps: keeps each layout's bio text within a sane
// design range (never comically huge, never unreadably small). The actual size
// within that range is picked by AutoFitBio below, from real measured layout —
// not guessed from a character-count formula.
const getBioSizeBounds = (templateId: string, isSquare: boolean) => {
  let minPx: number, maxPx: number;

  if (isSquare) {
    // Square poster renders at 418×418 px
    minPx = 8.0; maxPx = 20.0; // default (covers unmapped templates)
    if (templateId === 'whimsical') maxPx = 18.0;
    else if (templateId === 'comic') maxPx = 22.0;
    else if (templateId === 'editorial') maxPx = 22.0;
    else if (templateId === 'minimalist') maxPx = 22.0;
    else if (templateId === 'polaroid') maxPx = 22.0;
    else if (templateId === 'bio-only') { maxPx = 28.0; minPx = 13.5; }
    else if (templateId === 'two-photos' || templateId === 'extreme-duo') maxPx = 16.0;
    else if (templateId === 'comic-2-photos') maxPx = 16.0;
    else if (templateId === 'editorial-2-photos') maxPx = 21.0;
  } else {
    // Flyer poster renders at 378×489 px
    minPx = 7.5; maxPx = 17.0; // default
    if (templateId === 'whimsical') maxPx = 19.0;
    else if (templateId === 'comic') maxPx = 15.0;
    else if (templateId === 'editorial') maxPx = 18.0;
    else if (templateId === 'minimalist') maxPx = 16.0;
    else if (templateId === 'polaroid') maxPx = 16.0;
    else if (templateId === 'bio-only') { maxPx = 25.0; minPx = 12.5; }
    else if (templateId === 'two-photos' || templateId === 'extreme-duo') maxPx = 15.0;
    else if (templateId === 'comic-2-photos') maxPx = 15.0;
    else if (templateId === 'editorial-2-photos') maxPx = 17.0;
  }

  return { minPx, maxPx };
};

const lineHeightForSize = (size: number) => (size > 15 ? '1.4' : size > 12 ? '1.3' : '1.2');

// Renders bio text sized as large as it can be without overflowing its box.
// Binary-searches actual DOM measurements (the text's scrollHeight/scrollWidth
// vs. its parent's clientHeight/clientWidth) instead of estimating from a
// character-count formula, so it's precise for the real rendered box and the
// real word-wrapped line count. Only ever mutates its own font-size/line-height —
// never touches siblings, so it can't perturb a neighboring photo's layout.
// The parent box's own size must already be independent of this text (fixed
// height, or a flex-basis/grid-column share of a fixed-size ancestor) — every
// call site in this file satisfies that via CSS Grid columns or flex-basis
// ratios, with this component as the sole `flex-1` child in its stack.
const AutoFitBio: React.FC<{ text: string; className: string; templateId: string; isSquare: boolean; fontFamily?: string }> = ({ text, className, templateId, isSquare, fontFamily }) => {
  const { minPx, maxPx } = getBioSizeBounds(templateId, isSquare);
  ...
  React.useLayoutEffect(() => {
    ...
    // Web fonts (Fredoka, Playfair Display, Patrick Hand, etc.) can still be
    // loading when this first measurement runs, in which case it measures
    // against fallback-font metrics and the real font's line-wrapping can
    // differ once it loads. Re-measure once fonts are confirmed ready as a
    // correctness safety net — a no-op if they were already loaded.
  ...
  return (
    <p ref={ref} data-autofit-bio className={className} style={{ fontSize: `${fontSize.toFixed(1)}px`, lineHeight: lineHeightForSize(fontSize), ...(fontFamily ? { fontFamily } : {}) }}>
      {text}
    </p>
  );
};
...
export const PosterTemplates: React.FC<PosterTemplateProps> = ({ pet, settings, setPet, isPrintable = true }) => {
  ...
  const getBadgeStyle = () => {
    switch (settings.templateId) {
      ...
      case 'whimsical':
        return 'bg-[#fbbf24] text-[#0f5c3a] border-2 border-[#0f5c3a] tracking-wide font-handwritten font-black text-[12px] md:text-[13.5px] px-3.5 py-0.5 shadow-[2px_2px_0px_#0f5c3a] rounded-tl-[15px] rounded-br-[12px] rotate-[-1.5deg]';
      default:
          ...
          <div className={`${settings.templateId === 'polaroid' ? 'col-span-7' : 'col-span-6'} flex flex-col justify-between h-full py-0.5 relative border-l border-slate-100 pl-2 min-h-0 overflow-hidden`}>
            <div className="flex-1 w-full min-h-0 relative mb-1">
              {settings.templateId === 'polaroid' ? (
                ...
                <div className="bg-[#fffefe] p-1 pb-3 border-2 border-dashed border-[#2f9e44] shadow-md rotate-[-1.5deg] h-full w-full flex flex-col justify-between rounded-[8px_32px_12px_24px] relative">
                  ...
                  <div className="flex-1 h-0 w-full bg-stone-150 relative rounded-[4px_24px_8px_16px] overflow-hidden border border-[#2f9e44]/60">
                    {primaryPhoto ? (
                      <RepositionableImage src={primaryPhoto} alt={pet.name} pet={pet} setPet={setPet} />
                    ) : (
                  ...
                  <div className="min-w-0 col-span-2">
                    ...
                    <div className="grid grid-cols-3 gap-1 mt-0.5 text-center text-[8.5px] md:text-[9px] font-bold font-sans">
                      <div className="bg-white/90 border border-amber-200 px-0.5 py-0.5 rounded leading-none">
                        ...
                        <span className="text-stone-850 font-extrabold leading-none block mt-0.5 text-[8.5px] md:text-[9px] font-sans">{pet.goodWithDogs === 'yes' ? '✓ OK' : pet.goodWithDogs === 'no' ? '✗ No' : '✦ Selective'}</span>
                      </div>
                      <div className="bg-white/90 border border-amber-200 px-0.5 py-0.5 rounded leading-none">
                        <span className="text-[#a27b3e] text-[6.5px] md:text-[7.5px] block font-bold leading-none uppercase font-sans">CATS</span>
                        <span className="text-stone-850 font-extrabold leading-none block mt-0.5 text-[8.5px] md:text-[9px] font-sans">{pet.goodWithCats === 'yes' ? '✓ OK' : pet.goodWithCats === 'no' ? '✗ No' : '✦ Selective'}</span>
                      </div>
                      <div className="bg-white/90 border border-[#b3cca8] px-0.5 py-0.5 rounded leading-none">
                        ...
                        <span className="text-stone-850 font-extrabold leading-none block mt-0.5 text-[8.5px] md:text-[9px] font-sans">{pet.goodWithKids === 'yes' ? '✓ OK' : pet.goodWithKids === 'no' ? '✗ No' : '✦ Selective'}</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className={`w-full relative flex-1 ${settings.templateId === 'whimsical' ? 'max-h-[72%]' : settings.templateId === 'polaroid' ? 'max-h-[82%]' : settings.templateId === 'comic' ? 'max-h-[74%]' : settings.templateId === 'editorial' ? 'max-h-[72%]' : 'max-h-[70%]'} min-h-[115px] flex flex-col justify-center items-center ${settings.templateId === 'comic' ? 'p-1' : ''}`}>
            {settings.templateId === 'polaroid' ? (
              ...
              <div className="bg-[#fffefe] p-1 md:p-1.5 pb-1 md:pb-1 w-full max-w-full aspect-[0.68] flex flex-col rounded-[12px_44px_16px_36px] relative shrink-0 group shadow-xs rotate-[-1deg] border border-[#2f9e44]">
                ...
                <div className="flex-1 h-0 w-full bg-[#fcf9e8]/10 relative rounded-[4px_32px_8px_20px] overflow-hidden border border-[#2f9e44]/60">
                  {primaryPhoto ? (
                    <RepositionableImage src={primaryPhoto} alt={pet.name} pet={pet} setPet={setPet} />
                  ) : (
